...
from .utils.html_to_markdown_canlii import convert_file, html_to_markdown, refine_markdown
from .utils.jurisdiction import define_jurisprudential_network, define_legislative_network
...
def extract_citations(html_content):
    ...
    # Find all 'li' elements within the 'legislationLinks' div and extract 'data-path', excluding 'reflex'
    # Handle in case there is no 'legislationLinks' div
    if legislation_links_div:
        legislation_paths = [li['data-path'] for li in legislation_links_div.find_all('li') if 'data-path' in li.attrs and 'reflex' not in li['data-path']]
    else:
...
def save_file(request, submitted_text, context, url):
    ...
    if not file_path:
        ...
        file_path = f'../canlii_data/{jurisdiction}/{court_level}/{decision_year}/{primary_key}/{primary_key}.html'  # Set your default file path here
    ...
    try:
        ...
        context['message'] = f"File written."
    except IOError as e:
...
def extract_general_metadata(submitted_text):
    pass
...
def index(request):
    ...
    if request.method == 'POST':
        ...
        # General metadata extraction and cleaning

        # Extract the case and legislation keys and links
        ...
        style_of_cause_match = re.search(r'<meta name="lbh-title" content="([^"]+)"', submitted_text)
        citation_match = re.search(r'<meta name="lbh-citation" content="([^"]+)"', submitted_text)
        decision_date_match = re.search(r'<meta name="lbh-decision-date" content="([^"]+)"', submitted_text)
        language_match = re.search(r'<meta name="lbh-lang" content="([^"]+)"', submitted_text)
        court_level_match = re.search(r'<meta name="lbh-collection" content="([^"]+)"', submitted_text)
        jurisdiction_match = re.search(r'<meta name="lbh-jurisdiction" content="([^"]+)"', submitted_text)
        keywords_match = re.search(r'<meta name="lbh-keywords" content="([^"]+)"', submitted_text)
        subjects_match = re.search(r'<meta name="lbh-subjects" content="([^"]+)"', submitted_text)
        url_match = re.search(r'<meta name="lbh-document-url" content="([^"]+)"', submitted_text)
        
        # Returns the uncomplicated case information
        if style_of_cause_match and citation_match and decision_date_match and url_match and court_level_match and jurisdiction_match:
        ...
        if context['jurisdiction'] == "Saskatchewan" and int(context['decision_year']) >= 2016:
            ...
            context['case_type'] = (case_dict.get('case type', ''), case_dict.get('field', ''))
